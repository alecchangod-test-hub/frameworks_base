...
class ColorScheme(
    ...
    init {
        ...
        accent1 = TonalPalette(style.coreSpec.a1.shades(camSeed, luminanceFactor, chromaFactor), seedArgb)
        accent2 = TonalPalette(style.coreSpec.a2.shades(camSeed), seedArgb)
        accent3 = TonalPalette(style.coreSpec.a3.shades(camSeed), seedArgb)
        neutral1 = TonalPalette(style.coreSpec.n1.shades(camBgSeed,
                if (tintBackground) luminanceFactor else 1f,
                if (tintBackground) chromaFactor else 1f), seedArgb)
        neutral2 = TonalPalette(style.coreSpec.n2.shades(camBgSeed), seedArgb)
    }
